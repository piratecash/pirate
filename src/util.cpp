...
std::string ArgsManager::GetHelpMessage()
{
    ...
    for (auto& arg : m_available_args) {
        if (arg.first.first != last_cat) {
            ...
            else if (last_cat == OptionsCategory::POS)
                usage += HelpMessageGroup(_("Staking options:"));
            else if (last_cat == OptionsCategory::POW)
                usage += HelpMessageGroup(_("PoW mining:"));
        }
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty cosnta.conf if it does not excist
