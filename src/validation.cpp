                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(COSANTA): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    LogPrint(BCLog::BENCHMARK, "    - Cosanta specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeCosantaSpecific * 0.000001);
