...
class SpentIndexTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.stop_node(1)
        self.nodes[1].assert_start_raises_init_error(["-spentindex=0"], "You need to rebuild the database using -reindex to change -spentindex")
        self.start_node(1, ["-spentindex=0", "-reindex"])
        connect_nodes(self.nodes[0], 1)
        self.sync_all()
        self.stop_node(1)
        self.nodes[1].assert_start_raises_init_error(["-spentindex"], "You need to rebuild the database using -reindex to change -spentindex")
